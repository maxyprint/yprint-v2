  ...
  return (
    <div className="yprint-card">
      ...
      <p className="text-[rgba(0,0,0,0.6)] text-sm mb-6">
        Noch kein Konto?{' '}
        <Link href="/register" className="text-[#007aff] hover:underline font-medium">Jetzt registrieren</Link>
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)}
            required autoComplete="email" className="yprint-input" placeholder="deine@email.de" />
        </div>
        <div>
          <label htmlFor="password" className="block text-sm font-medium text-[#1d1d1f] mb-1.5">Passwort</label>
          <input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)}
            required autoComplete="current-password" className="yprint-input" placeholder="••••••••" />
        ...
        <button type="submit" disabled={loading || !turnstileToken} className="yprint-button yprint-button-primary w-full">
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="yprint-card h-64 animate-pulse" />}>
      <LoginForm />
    </Suspense>
  )
}
